# Mezzanine M1/M2 Runtime Persistence

## Scope

Mezzanine M1/M2 Runtime owns deterministic M1 and live M2 runtime separation owner. This document is package-local and is the persistence contract for `core/m1_m2_runtime` in `mezzanine`.
...
M1 memory-only deterministic facts by default; M2 explicit durable refs only.
...
```bash
cd core/m1_m2_runtime && mix test; root mix ci
```
...
Valid durability claims require explicit profile selection, adapter capability, migration or substrate preflight, redacted evidence, focused tests, repo QC, and a pushed commit. :ops_durable only in M2 after provider account, target attach, lease, operation policy, runtime substrate, and preflight proof.
...
M1 has no migrations. M2 durable owners must supply migration or substrate proof.
