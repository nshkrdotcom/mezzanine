defmodule Mezzanine.Control.Commands do
  @moduledoc """
  Operator control commands above the durable control and work domains.
  """
  ...
  def pause_work(tenant_id, work_object_id, operator_ref, payload \\ %{})
    ...
    with {:ok, work_object} <- fetch_work_object(tenant_id, work_object_id),
         {:ok, control_session} <- ensure_control_session(tenant_id, work_object),
  ...
  defp ensure_control_session(tenant_id, work_object) do
  ...
  defp control_session_for_work(tenant_id, work_object_id) do
    ControlSession
