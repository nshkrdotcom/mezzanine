...
Mezzanine Workflow Runtime owns Temporal workflow boundary, outbox persistence, worker registry, and runtime preflight owner. This document is package-local and is the persistence contract for `core/workflow_runtime` in `mezzanine`.
...
Memory store and memory outbox by default; SQL outbox and Temporal worker runtime are explicit opt-in.
...
```bash
cd core/workflow_runtime && mix test; root mix ci
```
...
Valid durability claims require explicit profile selection, adapter capability, migration or substrate preflight, redacted evidence, focused tests, repo QC, and a pushed commit. :ops_durable with Temporal substrate proof from just dev-* and SQL outbox migration proof.
