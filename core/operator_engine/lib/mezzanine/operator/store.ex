defmodule Mezzanine.Operator.Store do
  @moduledoc "Persistence facade for operator command state."

  alias Mezzanine.Operator.Store.AshPostgres
  ...
  defp reject_memory_profile!(opts) when is_list(opts) or is_map(opts) do
    ...
    if selected in @forbidden_profiles do
      raise ArgumentError, "production operator store cannot select #{inspect(selected)}"
    end
  end
end
