defmodule Mezzanine.Decisions.StoreTest do
  ...
  test "omitted options select the live durable decision store" do
    assert Store.adapter([]) == Mezzanine.Decisions.Store.AshPostgres
  ...
  test "production facade rejects memory profile selection" do
    for profile <- [:mickey_mouse, :memory_debug, "mickey_mouse", "memory_debug"] do
      assert_raise ArgumentError, ~r/production decision store cannot select/, fn ->
