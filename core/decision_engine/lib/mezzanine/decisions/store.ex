defmodule Mezzanine.Decisions.Store do
  @moduledoc "Persistence facade for decision ledger state."

  alias Mezzanine.Decisions.Store.AshPostgres
  ...
  defp reject_memory_profile!(opts) when is_list(opts) or is_map(opts) do
    ...
    if selected in @forbidden_profiles do
      raise ArgumentError, "production decision store cannot select #{inspect(selected)}"
    end
  end
end
