...
Mezzanine Core owns Mezzanine persistence-aware engine facade owner. This document is package-local and is the persistence contract for `core/mezzanine_core` in `mezzanine`.
...
```bash
cd core/mezzanine_core && mix test; root mix ci
```
