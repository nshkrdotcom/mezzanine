defmodule Mezzanine.Audit.Store do
  @moduledoc "Persistence facade for audit state."

  alias Mezzanine.Audit.Store.AshPostgres
  ...
  defp reject_memory_profile!(opts) when is_list(opts) or is_map(opts) do
    ...
    if selected in @forbidden_profiles do
      raise ArgumentError, "production audit store cannot select #{inspect(selected)}"
    end
  end
end
