defmodule Mezzanine.Audit.StoreTest do
  ...
  test "omitted options select the live durable audit store" do
    assert Store.adapter([]) == Mezzanine.Audit.Store.AshPostgres
  ...
  test "production facade rejects memory profile selection" do
    for profile <- [:mickey_mouse, :memory_debug, "mickey_mouse", "memory_debug"] do
      assert_raise ArgumentError, ~r/production audit store cannot select/, fn ->
