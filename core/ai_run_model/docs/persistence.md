...
Mezzanine AI Run Model owns Mezzanine persistence-aware engine facade owner. This document is package-local and is the persistence contract for `core/ai_run_model` in `mezzanine`.
...
```bash
cd core/ai_run_model && mix test; root mix ci
```
