...
Mezzanine Object Engine owns Mezzanine persistence-aware engine facade owner. This document is package-local and is the persistence contract for `core/object_engine` in `mezzanine`.
...
```bash
cd core/object_engine && mix test; root mix ci
```
...
- Schema owner: `Mezzanine.Objects.Repo`.
- Migration owner: `core/object_engine/priv/repo/migrations`.
- Migration preflight command: `Mezzanine.Objects.Store.preflight(profile: :integration_postgres, migration_proof: :present)`.
- Failure behavior: missing migration proof returns `{:error, {:missing_migration_proof, :objects}}` before object mutation.
- Tagged test command: `cd core/object_engine && mix test test/mezzanine/objects/store_test.exs`.
