defmodule Mezzanine.EvidenceLedger.Store do
  @moduledoc "Persistence facade for evidence ledger state."

  alias Mezzanine.EvidenceLedger.Store.AshPostgres
  ...
  defp reject_memory_profile!(opts) when is_list(opts) or is_map(opts) do
    ...
    if selected in @forbidden_profiles do
      raise ArgumentError, "production evidence store cannot select #{inspect(selected)}"
    end
  end
end
