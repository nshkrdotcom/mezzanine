defmodule Mezzanine.IntegrationBridge.EventTranslator do
  @moduledoc """
  Maps public-platform lifecycle events back into Mezzanine audit attrs.
  """
  ...
  def to_audit_attrs(event, attrs \\ %{}) when is_map(event) and is_map(attrs) do
    %{
      ...
      actor_ref: value(event, :actor_ref, "jido_integration"),
      payload: Map.drop(event, [:status, "status", :event_kind, "event_kind"]),
