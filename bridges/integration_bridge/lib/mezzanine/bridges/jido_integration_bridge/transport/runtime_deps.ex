defmodule Mezzanine.Bridges.JidoIntegrationBridge.Transport.RuntimeDeps do
  @moduledoc """
  Explicit runtime dependency holder for Mezzanine's Jido Integration transport.
  """
  ...
  @spec submit_invocation(t(), map(), keyword()) :: Transport.result()
  def submit_invocation(%__MODULE__{} = deps, request, opts \\ [])
      when is_map(request) and is_list(opts) do
    deps.transport.submit_invocation(request, Keyword.merge(deps.transport_opts, opts))
  end

  @spec read_invocation(t(), String.t(), keyword()) :: Transport.result()
  def read_invocation(%__MODULE__{} = deps, ref, opts \\ [])
      when is_binary(ref) and is_list(opts) do
    deps.transport.read_invocation(ref, Keyword.merge(deps.transport_opts, opts))
  end

  defp validate_transport(transport) when is_atom(transport) do
    case Code.ensure_loaded(transport) do
      {:module, ^transport} ->
        if function_exported?(transport, :submit_invocation, 2) and
             function_exported?(transport, :read_invocation, 2) do
          :ok
        else
