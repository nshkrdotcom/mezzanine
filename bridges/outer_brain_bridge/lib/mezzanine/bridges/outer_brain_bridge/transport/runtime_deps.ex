defmodule Mezzanine.Bridges.OuterBrainBridge.Transport.RuntimeDeps do
  @moduledoc """
  Explicit runtime dependency holder for Mezzanine's OuterBrain transport.
  """
  ...
  @spec compile_context(t(), map(), keyword()) :: Transport.result()
  def compile_context(%__MODULE__{} = deps, request, opts \\ [])
      when is_map(request) and is_list(opts) do
    deps.transport.compile_context(request, Keyword.merge(deps.transport_opts, opts))
  end

  @spec readback_context(t(), String.t(), keyword()) :: Transport.result()
  def readback_context(%__MODULE__{} = deps, ref, opts \\ [])
      when is_binary(ref) and is_list(opts) do
    deps.transport.readback_context(ref, Keyword.merge(deps.transport_opts, opts))
  end

  defp validate_transport(transport) when is_atom(transport) do
    case Code.ensure_loaded(transport) do
      {:module, ^transport} ->
        if function_exported?(transport, :compile_context, 2) and
             function_exported?(transport, :readback_context, 2) do
          :ok
        else
